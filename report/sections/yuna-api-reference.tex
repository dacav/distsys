...
\begin{description}
    ...
    \item[Failure probability]:
        as explained in section \Subsection{sub:TheCommunicationSystem},
    ...
    \begin{itemize}
        ...
        \item   Probability of having a faulty node \PrName{faulty}
                (\Const{faulty_prob} key), must be a value $0 \leq p \leq
                1$);

        \item   Probability, for a faulty node, of fail during a
                transmission \PrName{fc} (\Const{faulty_fail_prob} key),
    ...
    \begin{itemize}
        \item   The minimum delay (\Const{deliver_mindel} key), expressed
                in milliseconds;

        \item   The maximum delay (\Const{deliver_maxdel} key), expressed
                in milliseconds;

        \item   The probability distribution of the delay
                (\Const{deliver_dist} key), must be a distribution taking
                values in the interval $[0, 1]$. It is specified as an
                \Erlang\ \emph{tuple} \Const{\{M, F, A\}} providing the
                coordinates (\emph{module}, \emph{function name} and
                \emph{arguments}) of the function. Using \Const{\{random,
                uniform, []\}}, for instance, we obtain a time interval
                uniformly distributed between \Const{deliver_mindel} and
                \Const{deliver_maxdel}.

    \end{itemize}

    \item[Logger parameters] (\Const{log_args} key): only one parameter is
        required, namely a tuple \Const{\{Stream, NamePrefix\}}, where
        \Const{Stream} is a file descriptor (like \Const{standard_error},
        which is the default), and \Const{NamePrefix} is a string giving
...
\subsubsection{Parameters for Consensus}
    ...
    where
    \begin{itemize}
    \item   \Const{PeerParams} contains the parameters for the processes
            which will run the \emph{Consensus} algorithm;
    \item   \Const{NPeers} is the number of processes running the
            algorithm;
    \item   \Const{StatPeers} is the ratio $r \in [0, 1]$ of process which
            will report statistics (this is useful since allows to tune
            the better trade-off between statistics significance and
            overhead)
    \item   \Const{TBeacon} is the beacon time period (expressed in
            milliseconds).
    \end{itemize}
    ...
    \emph{Failure Detector} algorithm (\see{the original
    paper\cite{bib:FD}}).
